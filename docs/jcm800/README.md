...
## 番外編: どうやってチューニングしたか — WAV比較解析

> 「実際にどうやってパラメータを決めたの？」

**答え: 録音を比較して、足りない帯域を特定し、回路で補った。**

### 手順

```
① ギターを弾いて WAV を録音（ライン入力）
     │
     ▼
② ZOOM MS-50G+ の JCM800 を通した音も録音
     │
     ▼
③ 2つの WAV を FFT してスペクトル（周波数ごとの音量）を比較
     │
     ▼
④ 「どの帯域が何dB足りないか」を数値で把握
     │
     ▼
⑤ 足りない帯域を補うための回路設計を決定
```
...
`sound/prev.wav`（チューニング前のモデル出力）と `sound/after.wav`（ZOOM JCM800 の出力）を
FFT で比較解析した:

![WAV Comparison](09_wav_comparison.png)

### 帯域別の解析結果

| 帯域 | 周波数 | 差分 (ZOOM - 旧モデル) | 何をしたか |
|------|--------|----------------------|-----------|
| Sub | 20-80 Hz | -7.9 dB | ZOOMより低域が出すぎ → Sub Cut (-7dB@50Hz) を追加 |
| Low | 80-200 Hz | +3.8 dB | やや足りない → Bass EQの中心を100→150Hzに移動 |
| Low-Mid | 200-500 Hz | -0.1 dB | ほぼ同じ → 変更なし |
| Mid | 500-1000 Hz | -3.7 dB | ZOOMの方が凹んでいる → Mid スクープ(-1.9dB@600Hz)を設計 |
| Upper-Mid | 1-2 kHz | -0.9 dB | ほぼ同じ → 変更なし |
| Presence | 2-4 kHz | +1.6 dB | やや足りない → Presence EQ で補強 |
| Brilliance | 4-8 kHz | **+2.5 dB** | 足りない → Cathode Bypass (+5dB@800Hz) で歪み前ブースト |
| Air | 8-16 kHz | **+6.0 dB** | 大幅に足りない → 4x OVS でエイリアシング改善 |

### この表からわかったこと

1. **高域 (4-16kHz) が大幅に足りなかった**
   - v1 では歪み後に EQ で足していたが、ノイズも増幅していた
   - → 歪み前にカソードバイパスで高域をブーストする方式に変更
   - → 4x オーバーサンプリングで高域のエイリアシングを改善

2. **中域 (500-1000Hz) が多すぎた**
   - v1 の Mid ノブは真ん中で +4.9dB のブーストだった
   - → テーブルを再設計して、真ん中で -1.9dB のスクープに変更

3. **低域 (20-80Hz) が多すぎた**
   - → Sub Cut フィルタ (-7dB@50Hz) を追加
   - → DC ブロックの HPF を段ごとに上げる設計に変更
...
```
耳で聴く:  「なんか違う」「高域が足りない」「低域がモコモコ」
              ↓
数値で測る:  「4-8kHzが+2.5dB足りない」「50Hz以下が-7.9dB多い」
              ↓
回路で直す:  「Cathode Bypass +5dB@800Hz」「Sub Cut -7dB@50Hz」
```

耳だけだと「なんか違う」で止まってしまう。
FFT で数値化すると「何を」「何dB」変えればいいかが見える。

逆に、数値だけだと音楽的に良い音かどうかはわからない。
例えば Mid を -3.7dB 合わせることが「正解」かは、最終的には耳で判断する。

### 自分で試す方法

```bash
# 解析スクリプトを実行
python tools/analyze_wav_comparison.py sound/prev.wav sound/after.wav

# 自分の WAV ファイルで比較する場合
python tools/analyze_wav_comparison.py my_input.wav reference_output.wav
```

スクリプトは `docs/jcm800/09_wav_comparison.png` にグラフを出力する。
